'use strict';
module.exports = function createRateLimitChecker(ROCS_RATELIMIT) {
  // Configure rate limit. The following format is accepted for ROCS_RATELIMIT:
  ...
  var rateLimitConfig = /^(\d+) (\d+)(?:\s*$|\s+(.+)$)/.exec(ROCS_RATELIMIT);
  if (!rateLimitConfig) {
    ...
    unlimitedPattern.trim().split(/\s+/).forEach(function(unlimitedHost, i) {
      ...
      if (startsWithSlash || endsWithSlash) {
        if (unlimitedHost.length === 1 || !startsWithSlash || !endsWithSlash) {
          throw new Error('Invalid ROCS_RATELIMIT. Regex at index ' + i +

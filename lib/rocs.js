...
function withCORS(headers, request) {
  ...
  var corsMaxAge = request.rocsRequestState.corsMaxAge;
...
function proxyRequest(req, res, proxy) {
  var location = req.rocsRequestState.location;
  ...
  var proxyThroughUrl = req.rocsRequestState.getProxyForUrl(location.href);
  if (proxyThroughUrl) {
 ...
 * @param req {IncomingMessage} Incoming HTTP request, augmented with property rocsRequestState
 * @param req.rocsRequestState {object}
 * @param req.rocsRequestState.location {object} See parseURL
 * @param req.rocsRequestState.getProxyForUrl {function} See proxyRequest
 * @param req.rocsRequestState.proxyBaseUrl {string} Base URL of the CORS API endpoint
 * @param req.rocsRequestState.maxRedirects {number} Maximum number of redirects
 * @param req.rocsRequestState.redirectCount_ {number} Internally used to count redirects
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  var requestState = req.rocsRequestState;
...
function getHandler(options, proxy) {
  ...
  Object.keys(rocs).forEach(function(option) {
    if (Object.prototype.hasOwnProperty.call(options, option)) {
      rocs[option] = options[option];
    }
  });

  // Convert rocs.requireHeader to an array of lowercase header names, or null.
  if (rocs.requireHeader) {
    if (typeof rocs.requireHeader === 'string') {
      rocs.requireHeader = [rocs.requireHeader.toLowerCase()];
    } else if (!Array.isArray(rocs.requireHeader) || rocs.requireHeader.length === 0) {
      rocs.requireHeader = null;
    } else {
      rocs.requireHeader = rocs.requireHeader.map(function(headerName) {
        return headerName.toLowerCase();
      });
    }
  }
  var hasRequiredHeaders = function(headers) {
    return !rocs.requireHeader || rocs.requireHeader.some(function(headerName) {
      return Object.hasOwnProperty.call(headers, headerName);
    });
  };

  return function(req, res) {
    req.rocsRequestState = {
      getProxyForUrl: rocs.getProxyForUrl,
      maxRedirects: rocs.maxRedirects,
      corsMaxAge: rocs.corsMaxAge,
    };
    ...
    if (rocs.handleInitialRequest && rocs.handleInitialRequest(req, res, location)) {
      return;
    }
    ...
    if (!location) {
      ...
      showUsage(rocs.helpFile, cors_headers, res);
      return;
    }
    ...
    if (!hasRequiredHeaders(req.headers)) {
      ...
      res.end('Missing required request header. Must specify one of: ' + rocs.requireHeader);
      return;
    }

    var origin = req.headers.origin || '';
    if (rocs.originBlacklist.indexOf(origin) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if (rocs.originWhitelist.length && rocs.originWhitelist.indexOf(origin) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was not whitelisted by the operator of this proxy.');
      return;
    }

    var rateLimitMessage = rocs.checkRateLimit && rocs.checkRateLimit(origin);
    if (rateLimitMessage) {
    ...
    if (rocs.redirectSameOrigin && origin && location.href[origin.length] === '/' &&
    ...
    rocs.removeHeaders.forEach(function(header) {
      delete req.headers[header];
    });

    Object.keys(rocs.setHeaders).forEach(function(header) {
      req.headers[header] = rocs.setHeaders[header];
    });

    req.rocsRequestState.location = location;
    req.rocsRequestState.proxyBaseUrl = proxyBaseUrl;
